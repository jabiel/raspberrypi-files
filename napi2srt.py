...
class NapiProject():
    ...
    def getnapi(self, file):
        ...
        open('/tmp/napisy.7z', 'w').write(urllib.urlopen(str).read())
        ...
        if (not os.system('%s x -y -so -piBlm8NTigvru0Jr0 /tmp/napisy.7z 2>/dev/null > "%s"' % (p7zip, subtitle))):
            os.remove('/tmp/napisy.7z')
            return 0
        else:
            os.remove(subtitle)
            os.remove('/tmp/napisy.7z')
